...
class TradingEngineProcessor implements Processor{
	...
	public TradingEngineProcessor(){
		Stock ibm=new Stock("IBM",300);
		ibm.addInterestedStat("bidAmount");
		ibm.addInterestedStat("bidAmountAverage");
		ibm.addInterestedStat("askAmount");
		ibm.addInterestedStat("askAmountAverage");
		ibm.setTradingRule("bidAmount",40.0, "LARGERTHAN", "BUY");
		ibm.setTradingRule("askAmount",60.0, "LARGERTHAN", "SELL");
		
		Stock orcl=new Stock("ORCL",200);
		orcl.addInterestedStat("bidAmount");
		orcl.addInterestedStat("bidAmountAverage");
		orcl.addInterestedStat("askAmount");
		orcl.addInterestedStat("askAmountAverage");
		orcl.setTradingRule("bidAmount",40.0, "LARGERTHAN", "BUY");
		orcl.setTradingRule("askAmount",60.0, "LARGERTHAN", "SELL");
		
		rootPortfolio.add(ibm);
		rootPortfolio.add(orcl);
	}
    ...
    public static void main(String args[]) throws Exception {
        ...
        context.addRoutes(new RouteBuilder() {
            public void configure() {
            	...
            	from("jms:topic:IBM_Stats").process(tep).to("jms:queue:trading_engine2_trash");
            	from("jms:topic:ORCL_Stats").process(tep).to("jms:queue:tranding_engine2_trash");
            	from("jms:topic:MSFT_Stats").process(tep).to("jms:queue:tranding_engine2_trash");
            }
        });
